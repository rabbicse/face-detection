...
from detector.utils.bbox_nms import multiclass_nms
from detector.utils.iou2d_calculator import bbox_overlaps
from detector.utils.transforms import distance2bbox, kps2distance, bbox2distance, distance2kps
from detector.utils.utils import normal_init, multi_apply, unmap, reduce_mean, images_to_levels, anchor_inside_flags
...
class SCRFDHead(AnchorHead):
    ...
    def _get_bboxes_single(self,
        ...
        for cls_score, bbox_pred, stride, anchors in zip(
            ...
            # nms_pre = cfg.get('nms_pre', -1)
            if nms_pre and nms_pre > 0 and scores.shape[0] > nms_pre:
                max_scores, _ = scores.max(dim=1)
                _, topk_inds = max_scores.topk(nms_pre)
                anchors = anchors[topk_inds, :]
                bbox_pred = bbox_pred[topk_inds, :]
